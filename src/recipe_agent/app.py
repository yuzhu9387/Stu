...
from recipe_agent.api.v1.kitchen import router as kitchen_router
from recipe_agent.api.v1.kitchen_ai import router as kitchen_ai_router
from recipe_agent.api.v1.kitchen_mcp import router as kitchen_mcp_router
...
def web_origins(origin: str) -> list[str]:
    """Allow either loopback spelling on the configured local frontend port."""
    origins = [origin.rstrip("/")]
    parsed = urlsplit(origin)
    if parsed.hostname in {"localhost", "127.0.0.1"}:
        alias = "127.0.0.1" if parsed.hostname == "localhost" else "localhost"
        port = f":{parsed.port}" if parsed.port is not None else ""
        origins.append(f"{parsed.scheme}://{alias}{port}")
    return origins
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    app.add_middleware(
        CORSMiddleware,
        allow_origins=web_origins(resolved_settings.web_origin),
    ...
    app.include_router(kitchen_ai_router)
    app.include_router(kitchen_mcp_router)
