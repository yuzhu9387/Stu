...
class Settings(BaseSettings):
    ...
    # A week is drafted in the background, so it may take minutes; medium
    # reasoning ran past four minutes on a real household, low takes about two.
    kitchen_generation_timeout_seconds: float = Field(default=420.0, gt=0, le=600)
    kitchen_generation_reasoning_effort: Literal["low", "medium", "high"] = "low"
    kitchen_chat_reasoning_effort: Literal["none", "low", "medium"] = "low"
    # Staged cutover: serve the workspace from the normalised tables instead of
    # the JSON aggregate. Every section is now projected on write, and both
    # paths render the same document for a backfilled household. Still defaults
    # off so the switch is a deliberate, observed change: enable it, watch, and
    # fall back by setting it false again. A household must be backfilled
    # first, or the tables will be empty.
    kitchen_relational_read: bool = True
