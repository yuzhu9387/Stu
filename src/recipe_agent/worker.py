...
async def run() -> None:
    ...
    kitchen_task = asyncio.create_task(
        kitchen_generation_loop(session_factory, settings), name="kitchen-generation"
    )
    try:
        while True:
...
async def kitchen_generation_loop(
    session_factory: async_sessionmaker[AsyncSession], settings: Settings
) -> None:
    """Independent scheduler: slow AI never blocks transactional-outbox polling."""
    while True:
        try:
            completed = await run_due_kitchen_jobs(session_factory, settings)
            if completed:
                print(
                    render_log({"event": "kitchen.generation.completed", "count": completed}),
                    flush=True,
                )
        except asyncio.CancelledError:
            raise
        except Exception:
            # Provider details, prompts and household data must not enter operational logs.
            print(render_log({"event": "kitchen.scheduler.failed"}), flush=True)
