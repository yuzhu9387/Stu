...
def test_card_callback_route_returns_lark_200_without_leaking_action_error(
    ...
    response = client_factory(app).post(
...
def test_live_event_lease_returns_retryable_response_instead_of_acknowledging(
    client_factory,
) -> None:
    inbound = LarkInboundService(
        identity=IdentityResolver({}),
    ...
    response = client_factory(app).post("/webhooks/lark/events", json=_message_payload())
