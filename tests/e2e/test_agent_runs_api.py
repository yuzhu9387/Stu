...
def test_authenticated_submission_returns_accepted_and_is_idempotent(
...
def test_run_endpoints_require_authentication(client_factory, tmp_path) -> None:
    database_url = f"sqlite+aiosqlite:///{tmp_path / 'agent-auth.db'}"
    asyncio.run(_create_schema(database_url))
    client = client_factory(create_app(_settings(database_url)))
...
def test_account_cannot_read_or_continue_another_accounts_private_run(
    client_factory, tmp_path
) -> None:
...
def test_web_supplied_missing_conversation_is_not_created(client_factory, tmp_path) -> None:
