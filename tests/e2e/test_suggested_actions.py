...
def test_natural_language_completion_only_issues_consent_and_does_not_mutate(
...
def test_authenticated_action_click_executes_once_and_returns_typed_result(
    client_factory, tmp_path
) -> None:
...
def test_save_recipe_mutation_is_idempotent_by_action_id_after_worker_crash(
    client_factory, tmp_path
) -> None:
...
def test_worker_recovers_crash_after_mutation_commit_without_duplicate_write(
    client_factory, tmp_path
) -> None:
...
def test_create_and_replace_plan_mutations_are_idempotent_by_action_id(
    client_factory, tmp_path
) -> None:
...
def test_create_plan_retry_reads_receipt_before_recommender(client_factory, tmp_path) -> None:
...
def test_replace_plan_retry_reads_receipt_before_missing_plan(client_factory, tmp_path) -> None:
...
def test_create_share_is_idempotent_and_never_persists_plaintext_delivery(
    client_factory, tmp_path
) -> None:
...
def test_web_executes_completed_run_action_by_scoped_id_without_receiving_token(
    client_factory, tmp_path
) -> None:
    database_url = f"sqlite+aiosqlite:///{tmp_path / 'web-action-id.db'}"
    asyncio.run(_create_schema(database_url))
    app = create_app(_settings(database_url))
    owner = client_factory(app)
    intruder = client_factory(app)
