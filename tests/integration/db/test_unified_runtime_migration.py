...
class PostgresDatabase:
    def __init__(self, database_name: str, admin_url: str = POSTGRES_ADMIN_URL) -> None:
...
def postgres_database() -> Iterator[PostgresDatabase]:
    ...
    configured_test_url = os.environ.get("RECIPE_AGENT_TEST_DATABASE_URL")
    admin_url = (
        make_url(configured_test_url)
        .set(drivername="postgresql", database="postgres")
        .render_as_string(hide_password=False)
        if configured_test_url
        else POSTGRES_ADMIN_URL
    )
    try:
        admin_connection = psycopg.connect(admin_url, autocommit=True)
    except psycopg.OperationalError:
        if configured_test_url:
            pytest.fail("The configured PostgreSQL test server is unavailable")
        pytest.skip("RECIPE_AGENT_TEST_DATABASE_URL is unset and local PostgreSQL is unavailable")
    ...
    database = PostgresDatabase(database_name, admin_url)
    try:
        yield database
    finally:
