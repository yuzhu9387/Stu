...
def test_disabled_lark_is_closed_and_live_lark_uses_real_handler(client_factory, tmp_path) -> None:
    disabled = create_app(_settings(f"sqlite+aiosqlite:///{tmp_path / 'disabled-lark.db'}"))
    disabled_response = client_factory(disabled).post(
...
async def test_composed_run_worker_executes_model_and_persists_action_references(
    ...
    async with app.router.lifespan_context(app):
        ...
        assert not await run_agent_job(
            runtime.agent_run_repository, runtime.agent_run_service, run_id
        )
