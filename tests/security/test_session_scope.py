...
def test_session_cookie_resolves_account_and_family_scope(client_factory, tmp_path) -> None:
...
def test_expired_or_membership_inconsistent_sessions_are_unauthenticated(
...
def test_delete_session_invalidates_server_record_and_clears_cookie(
    client_factory, tmp_path
) -> None:
...
def test_magic_link_token_is_exposed_only_in_development(client_factory, tmp_path) -> None:
    ...
    asyncio.run(_create_schema(database_url))
    client = client_factory(create_app(_settings(database_url, environment="development")))
...
def test_owner_family_and_lark_link_endpoints_use_session_scope(client_factory, tmp_path) -> None:
...
def test_login_then_accept_invite_uses_authenticated_account_only(client_factory, tmp_path) -> None:
    database_url = f"sqlite+aiosqlite:///{tmp_path / 'accept-api.db'}"
    asyncio.run(_create_schema(database_url))
    app = create_app(_settings(database_url, environment="development"))
    owner_client = client_factory(app)
    member_client = client_factory(app)
...
def test_invite_accept_rejects_account_id_and_requires_session(client_factory, tmp_path) -> None:
    ...
    unauthenticated = client_factory(app).post(
...
def test_invite_accept_preserves_owned_preference_and_share(client_factory, tmp_path) -> None:
...
def test_session_cookie_security_attributes_follow_environment(client_factory, tmp_path) -> None:
    development_url = f"sqlite+aiosqlite:///{tmp_path / 'cookie-development.db'}"
    asyncio.run(_create_schema(development_url))
    development = client_factory(create_app(_settings(development_url, environment="development")))
